    ...
    <article style={card}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ ...muted, marginTop: 4, fontSize: 13 }}>
            {row.price} $ · acheté le {new Date(row.created_at).toLocaleString("fr-FR")}
          </div>
        </div>
      ...
      {row.status === "delivre" && row.delivered_at && (
        <div style={{ ...muted, marginTop: 10, fontSize: 12 }}>
          ...
          {row.delivered_by_username ? ` par ${row.delivered_by_username}` : ""} · +{row.points_earned} points crédités.
        </div>
      )}
      {row.status === "paiement_initie" && (
        <div style={{ ...muted, marginTop: 10, fontSize: 12 }}>
          +{row.points_earned} points seront crédités une fois le billet attribué par un agent.
        </div>
      )}
    </article>
  );

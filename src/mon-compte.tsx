 ...
 * ------------------------------------------------------------------ */
/* Le thème clair/sombre est piloté par la classe `.dark` sur <html>
 * (voir ThemeToggle). Cette page utilisait autrefois des couleurs figées :
 * elle ne suivait donc jamais le thème choisi et restait toujours sombre.
 * On passe ici par des variables CSS avec une valeur claire par défaut
 * (voir le bloc `css` plus bas) et une surcharge sombre, comme le reste du site. */
const C = {
  bg: "var(--tte-bg)",
  bg2: "var(--tte-bg2)",
  line: "var(--tte-line)",
  ...
  muted: "var(--tte-muted)",
  accent: "#4B92DD",
  accentSoft: "var(--tte-accent-soft)",
  ok: "var(--tte-ok)",
  warn: "var(--tte-warn)",
  off: "var(--tte-off)",
  danger: "var(--tte-danger)",
};
    ...
    <article className="tte-card" style={ticketCard}>
      ...
      {tolerance && (
        <div style={{ padding: "0 20px 14px" }}>
          {tolerance.expired ? (
            <div style={{ ...notice, background: "rgba(248,113,113,0.1)", borderColor: "rgba(248,113,113,0.35)", color: "var(--tte-danger)", padding: "10px 12px", fontSize: 12.5 }}>
...
function RewardTierCard({
  ...
  return (
    <article className="tte-card" style={{ ...card, display: "grid", gap: 12 }}>
      ...
      <div style={{ height: 7, borderRadius: 999, background: "var(--tte-surface-1)", overflow: "hidden" }}>
        <div
          style={{
            height: "100%",
            width: `${pct}%`,
...
const card: React.CSSProperties = {
  background: `linear-gradient(180deg, var(--tte-surface-1), var(--tte-surface-2))`,
...
const ticketCard: React.CSSProperties = {
  position: "relative",
  background: `linear-gradient(180deg, var(--tte-surface-4b), var(--tte-surface-2))`,
  border: `1px solid ${C.line}`,
  borderRadius: 16,
  overflow: "hidden",
  paddingLeft: 4,
...
const perfLine: React.CSSProperties = {
  display: "block",
  height: 1,
  background: "repeating-linear-gradient(90deg, var(--tte-perf-line) 0 6px, transparent 6px 12px)",
};
...
:root {
  --tte-bg: #f3f6fb;
  --tte-bg2: #e7edf6;
  --tte-line: rgba(15,23,42,0.11);
  --tte-text: #0f172a;
  --tte-muted: #57647c;
  --tte-off: #7c8898;
  --tte-accent-soft: rgba(75,146,221,0.10);
  --tte-ok: #0f8f5f;
  --tte-warn: #a35a09;
  --tte-danger: #dc2626;
  --tte-surface-1: rgba(15,23,42,0.05);
  --tte-surface-2: rgba(15,23,42,0.015);
  --tte-surface-3: rgba(15,23,42,0.04);
  --tte-surface-4: rgba(15,23,42,0.045);
  --tte-surface-4b: rgba(15,23,42,0.055);
  --tte-header-bg: rgba(255,255,255,0.78);
  --tte-hover-bg: rgba(15,23,42,0.07);
  --tte-notice-text: #35415a;
  --tte-perf-line: rgba(15,23,42,0.18);
  --tte-skel-1: rgba(15,23,42,0.09);
  --tte-skel-2: rgba(15,23,42,0.03);
  --tte-skel-border: rgba(15,23,42,0.09);
  --tte-shadow: rgba(15,23,42,0.16);
}
.dark {
  --tte-bg: #0B1220;
  --tte-bg2: #111C2E;
  --tte-line: rgba(255,255,255,0.09);
  --tte-text: #E2E8F0;
  --tte-muted: #93A4BC;
  --tte-off: #7C8CA5;
  --tte-accent-soft: rgba(75,146,221,0.12);
  --tte-ok: #34D399;
  --tte-warn: #FBBF24;
  --tte-danger: #F87171;
  --tte-surface-1: rgba(255,255,255,0.045);
  --tte-surface-2: rgba(255,255,255,0.015);
  --tte-surface-3: rgba(255,255,255,0.035);
  --tte-surface-4: rgba(255,255,255,0.04);
  --tte-surface-4b: rgba(255,255,255,0.05);
  --tte-header-bg: rgba(11,18,32,0.75);
  --tte-hover-bg: rgba(255,255,255,0.06);
  --tte-notice-text: #CBD8E8;
  --tte-perf-line: rgba(255,255,255,0.22);
  --tte-skel-1: rgba(255,255,255,0.06);
  --tte-skel-2: rgba(255,255,255,0.02);
  --tte-skel-border: rgba(255,255,255,0.07);
  --tte-shadow: rgba(0,0,0,0.9);
}
@keyframes tte-spin { to { transform: rotate(360deg); } }
@keyframes tte-pulse { 0%,100% { opacity: .45 } 50% { opacity: .8 } }
.tte-skeleton {
  background: linear-gradient(180deg, var(--tte-skel-1), var(--tte-skel-2));
  border: 1px solid var(--tte-skel-border);
...
.tte-btn:hover { background: var(--tte-hover-bg); transform: translateY(-1px); border-color: rgba(75,146,221,0.5); }
.tte-btn:disabled { opacity: .6; cursor: default; transform: none; }
.tte-navlink { transition: color .18s ease, background .18s ease; }
.tte-navlink:hover { color: var(--tte-text); background: var(--tte-hover-bg); }
.tte-card { transition: transform .2s ease, border-color .2s ease, box-shadow .2s ease; }
.tte-card:hover { transform: translateY(-2px); border-color: rgba(75,146,221,0.35); box-shadow: 0 18px 40px -28px var(--tte-shadow); }

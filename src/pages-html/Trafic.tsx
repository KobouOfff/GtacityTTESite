...
const statusLabels: Record<string,string> = {on_time:"À l’heure",boarding:"Embarquement",delayed:"Retard",platform_changed:"Quai modifié",cancelled:"Supprimé"};
...
export default function TraficPage() {
  ...
  const lineState = (line: string) => {
    ...
    if (publication?.severity === "alert" || updates.some((record) => record.status === "cancelled")) return ["Interrompu / perturbé","traffic-alert"];
    if (publication || updates.length) return ["Perturbations","traffic-warn"];
    return ["Trafic normal","traffic-ok"];
  };
  ...
  return <div className="transit-page">
    ...
    <section className="transit-hero"><div className="transit-wrap">
      <div className="transit-crumb"><a href="/">Accueil</a> › Info trafic</div>
      <h1>Info trafic en direct</h1>
      <p>État des lignes, incidents publiés par le Centre de Régulation et modifications des trains programmés.</p>
    </div></section>
    <main className="transit-main transit-wrap">
      {error && <div className="transit-card traffic-item alert"><strong>Actualisation momentanément indisponible.</strong> Les informations affichées peuvent être incomplètes.</div>}
      <section className="transit-section"><h2>État du réseau</h2><div className="traffic-summary">
      ...
      <section className="transit-section"><h2>Alertes voyageurs</h2><div className="traffic-list">
        {!traffic.length && <div className="transit-card transit-empty">Aucune alerte active. Le trafic est normal.</div>}
        {traffic.map((item) => <article key={item.id} className={`transit-card traffic-item ${item.severity}`}>
          <h3>{item.line ? `Ligne ${item.line} — ` : ""}{item.title}</h3><p>{item.message}</p>
          <small>Publié par le Centre de Régulation{item.until ? ` · valable jusqu’au ${new Date(item.until).toLocaleString("fr-FR")}` : ""}</small>
        </article>)}
      </div></section>
      <section className="transit-section"><h2>Trains modifiés aujourd’hui</h2><div className="public-table-wrap"><table className="public-table">
        <thead><tr><th>Horaire</th><th>Ligne</th><th>Train</th><th>Destination</th><th>Voie</th><th>État</th></tr></thead>
        <tbody>{affected.length ? affected.map((item) => <tr key={item.id}><td><strong>{item.departure}</strong>{item.delayMinutes > 0 && <small style={{display:"block",textDecoration:"line-through"}}>{item.scheduledDeparture}</small>}</td><td><span className="line-pill" style={{background:colors[item.line]}}>{item.line}</span></td><td>{item.serviceName}</td><td>{item.destination}{item.message && <small style={{display:"block"}}>{item.message}</small>}</td><td>{item.platform}</td><td><span className={`status-pill ${item.status}`}>{statusLabels[item.status]}</span></td></tr>) :
          <tr><td colSpan={6} className="transit-empty">Aucun train modifié aujourd’hui.</td></tr>}</tbody>
      </table></div></section>
    </main>
  </div>;
}

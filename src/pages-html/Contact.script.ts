...
(function(){
  ...
  var LABELS={remboursement:'Remboursement',info:'Information voyageur',presse:'Presse & médias',objets:'Objets trouvés',accessibilite:'Accessibilité / PMR',reclamation:'Réclamation',suggestion:'Suggestion',autre:'Autre demande'};
  var DELAYS={remboursement:'sous 15 jours ouvrés',info:'sous 3 jours ouvrés',presse:'sous 24 à 48 heures',objets:'sous 5 jours ouvrés',accessibilite:'sous 5 jours ouvrés',reclamation:'sous 10 jours ouvrés',suggestion:'sous 10 jours ouvrés',autre:'sous 5 jours ouvrés'};
  var HINTS={
  ...
  function setCat(cat,scroll){
    ...
    syncBlocks(cat);
    if(cat && LABELS[cat]){
      formTitle.textContent='Demande · '+LABELS[cat];
      formHint.innerHTML=HINTS[cat]||'';
    }else{
      formTitle.textContent='Nouvelle demande';
      formHint.textContent='Sélectionnez un motif ci-dessus, puis remplissez le formulaire.';
    }
    ...
    form.addEventListener('submit', async function(e){
      ...
      if(problems.length){
        if(problems.indexOf('discord')>-1){
          errTxt.textContent='Connectez-vous avec Discord pour envoyer votre demande.';
          err.classList.add('show');
          var gate=document.getElementById('steamGate');
          if(gate) gate.scrollIntoView({behavior:'smooth',block:'center'});
        } else {
          errTxt.textContent='Merci de compléter les champs obligatoires marqués d\\u2019un astérisque.';
          err.classList.add('show');
      ...
      try{
        ...
        if(!res.ok || !payload.ok){
          errTxt.textContent = payload && payload.reason === 'not_logged_in'
            ? 'Ta session Discord a expiré. Reconnecte-toi pour envoyer la demande.'
            : 'Impossible d\\u2019enregistrer la demande ('+((payload&&payload.reason)||res.status)+').';
          err.classList.add('show');
          err.scrollIntoView({behavior:'smooth',block:'center'});
          return;
        }
        ...
        document.getElementById('sumDelay').textContent=DELAYS[cat]||'sous 5 jours ouvrés';

        form.style.display='none';
        var s=document.getElementById('success'); s.classList.add('show');
        s.scrollIntoView({behavior:'smooth',block:'start'});
      } catch(ex){
        errTxt.textContent='Erreur réseau : '+ex.message;
        err.classList.add('show');
      } finally {
